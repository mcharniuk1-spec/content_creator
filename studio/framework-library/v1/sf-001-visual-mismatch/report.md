# SF-001 — Visual Mismatch

Truth state: `ORIGINAL_LOCAL_PREVISUALIZATION / EXTERNAL_PROVIDER_NOT_RUN / OWNER_REVIEW_PENDING`

## Purpose

Break an expected visual pattern This is a 4-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-001`
- Sources: `SRC-WEB-001`
...
|---|---|---|---|---|---|
| 0–1s | anomaly | full | establish_context | WAIT | `frames/SF-001-F01.svg` |
| 1–2s | name tension | voice_over | establish_context | WHAT CHANGED? | `frames/SF-001-F02.svg` |
| 2–3s | connect | voice_over | establish_context | CONNECT IT | `frames/SF-001-F03.svg` |
| 3–4s | clean exit | return_cut | establish_context | RETURN | `frames/SF-001-F04.svg` |

## Production prompt

Create a vertical 9:16 4-second clean-room insert titled “Visual Mismatch.” Use one meaningful visual state per second. Hook mechanism: `visual_anomaly`. Viewer job: `identify_problem`. Visual grammar: `kinetic_type`. B-roll function: `establish_context`. Proof form: `observed_artifact`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
