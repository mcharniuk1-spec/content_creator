# SF-023 — Bypassed Approval

Truth state: `ORIGINAL_LOCAL_PREVISUALIZATION / EXTERNAL_PROVIDER_NOT_RUN / OWNER_REVIEW_PENDING`

## Purpose

Show risk from bypassing a gate This is a 8-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-023`
- Sources: `SRC-WEB-004`
...
|---|---|---|---|---|---|
| 0–1s | failure result | absent | contrast | THIS FAILED | `frames/SF-023-F01.svg` |
| 1–2s | first cause | absent | contrast | FIRST CAUSE | `frames/SF-023-F02.svg` |
| 2–3s | chain reaction | full | contrast | THEN | `frames/SF-023-F03.svg` |
| 3–4s | human impact | voice_over | contrast | IMPACT | `frames/SF-023-F04.svg` |
| 4–5s | control | voice_over | contrast | CONTROL | `frames/SF-023-F05.svg` |
| 5–6s | bridge detail | voice_over | contrast | BRIDGE | `frames/SF-023-F06.svg` |
| 6–7s | proof hold | voice_over | contrast | HOLD | `frames/SF-023-F07.svg` |
| 7–8s | return to speaker | return_cut | contrast | BACK TO YOU | `frames/SF-023-F08.svg` |

## Production prompt

Create a vertical 9:16 8-second clean-room insert titled “Bypassed Approval.” Use one meaningful visual state per second. Hook mechanism: `consequence`. Viewer job: `assess_risk`. Visual grammar: `diagram_build`. B-roll function: `contrast`. Proof form: `calculation`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
