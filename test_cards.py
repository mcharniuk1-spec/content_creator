...
# §13.7 negatives: strong reels that must never reach the pool
for code, user, pk in (('E1CODE001', 'e1', 201), ('E2CODE001', 'e2', 202), ('E3CODE001', 'e3', 203)):
    con.execute("INSERT INTO accounts (pk,username,status) VALUES (?,?,'active')", (pk, user))
    con.execute("""INSERT INTO reels (snapshot_id,code,pk_user,username,ts,play,dur,cap)
        VALUES (?,?,?,?,?,?,?,?)""", (sid, code, pk, user, RECENT_TS, 9000, 60.0, 'english caption, strong reel'))
    con.execute("""INSERT INTO scores (snapshot_id,code,eligible,author_median_play,resh_1k,save_1k,weights)
        VALUES (?,?,1,1000,400,400,'ig')""", (sid, code))
    con.execute("INSERT INTO topics (code,topic,source) VALUES (?,'Новости моделей и лабораторий','manual')", (code,))
# e1: caption only, no transcript, frames present
for i in range(9):
...
# §13.7 (Misha, 13 Sep evening): no transcript, no frames, or not English -> not a card, however strong
...
eq('drop counters explain it', cards._pool.dropped, {'no_transcript': 2, 'no_frames': 1, 'not_english': 1, 'missing_intent_metrics': 0})
...
con.execute("UPDATE scores SET save_1k=NULL WHERE code='U1CODE001'")
eq('unknown saves never become zero in combined ranking', 'U1CODE001' in {r['code'] for r in cards._pool(con, TODAY)}, False)
eq('missing metrics have an explicit exclusion counter', cards._pool.dropped['missing_intent_metrics'] > 0, True)
con.close(); os.remove(TMP); os.remove(_ta_e2)
