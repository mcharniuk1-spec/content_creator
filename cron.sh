...
# Будильник на весь прогон — предохранитель от зависания, а не рабочий лимит.
# Был 7200 (2 ч) и 14 сентября 2026 обрезал нормальный прогон на шаге 3. Рабочие
# сроки теперь живут внутри run.py у каждого тяжёлого шага (DEEP_BUDGET_S).
timeout 21600 .venv/bin/python run.py --yes
code=$?
echo "── прогон завершился с кодом $code ──"

# Результат собирается и при неполном прогоне (решение Миши, 14 сентября 2026). Здесь
# стояло `if [ $code -eq 0 ]` — «прогон завершился идеально, иначе не публикуем ничего».
# На конвейере из тринадцати шагов, где каждый ходит в интернет, это правило означает
# «ничего» слишком часто: срыв срока на разборе обнулил работу всех остальных шагов,
# хотя материала на карточки уже хватало. Углы, Notion и страницы собираются по тому,
# что реально лежит в базе, а неполнота видна в итогах по шагам и в статусе прогона.
if [ $code -ne 0 ]; then
    echo "прогон неполный (код $code) — результат собираем по тому, что успели"
fi
