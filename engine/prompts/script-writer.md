...
13. Audience language (Misha's decision, 13 Sep 2026; evidence `reports/audience/07-reddit-pass.md`, 101
    real threads; personas `docs/AUDIENCE_PERSONAS_2026-09-12.md`). Every card names its persona (Mary /
    Bruce / Marta) and the exact question it answers, in the viewer's own words from the phrasebank in
    §2 of that report. Rules: (a) name the job, not the technology — "the same 20 questions on WhatsApp",
    never "support automation"; no LLM / RAG / agent / workflow / MCP / API / "AI-powered" in the hook or
    the first 10 s; "AI" alone is fine, the tool name comes after the job is shown; (b) no machine-text
    tells anywhere: "quietly", "it's not X, it's Y", em-dashes, "here's what ChatGPT said"; (c) every card
    answers "is it right the first time?" with a visible human check on screen (draft queue, one-tap
    approve, source doc with owner and review date, 20 test questions before go-live); (d) cost is a
    monthly number plus the point where it stops paying ($20 is the audience's reference price, $100/mo is
    rejected, $200 only when limits are hit and hours are counted); "free" is not an answer; (e) never
    promise replacing people — say "hours back"; (f) customer-facing AI (chatbots, AI receptionist, AI
    images, AI copy) is publicly rejected by this audience; our territory is back-office, the one allowed
    exception is after-hours intake; (g) prices are never generated by the model — it reads a price sheet;
    say so on camera when the card touches quotes; (h) the CTA artefact (the comment-keyword deliverable)
    must be a real one-page thing the viewer keeps: test-question sheet, price-sheet template, customer
    exception card, decision-minutes template, cost-per-run sheet, four-question AI policy.
