# SF-058 — Screen-to-Real Match

Truth state: `ORIGINAL_LOCAL_PREVISUALIZATION / EXTERNAL_PROVIDER_NOT_RUN / OWNER_REVIEW_PENDING`

## Purpose

Match an interface state to physical action This is a 7-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-058`
- Sources: `SRC-WEB-008`
- Evidence: `EVID-WEB-012`
...
| Axis | Value |
|---|---|
| `hook_mechanism` | `direct_promise` |
| `viewer_job` | `identify_problem` |
| `broll_function` | `prove_state` |
...
|---|---|---|---|---|---|
| 0–1s | establish | full | prove_state | CONTEXT | `frames/SF-058-F01.svg` |
| 1–2s | action one | voice_over | prove_state | STEP 1 | `frames/SF-058-F02.svg` |
| 2–3s | action two | voice_over | prove_state | STEP 2 | `frames/SF-058-F03.svg` |
| 3–4s | detail | voice_over | prove_state | DETAIL | `frames/SF-058-F04.svg` |
| 4–5s | completed state | voice_over | prove_state | DONE | `frames/SF-058-F05.svg` |
| 5–6s | bridge detail | voice_over | prove_state | BRIDGE | `frames/SF-058-F06.svg` |
| 6–7s | A-roll return | return_cut | prove_state | RETURN | `frames/SF-058-F07.svg` |

## Production prompt

Create a vertical 9:16 7-second clean-room insert titled “Screen-to-Real Match.” Use one meaningful visual state per second. Hook mechanism: `direct_promise`. Viewer job: `identify_problem`. Visual grammar: `mock_screen_capture`. B-roll function: `prove_state`. Proof form: `observed_artifact`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
