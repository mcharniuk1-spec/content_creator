# SF-045 — Calculation Reveal

Truth state: `ORIGINAL_LOCAL_PREVISUALIZATION / EXTERNAL_PROVIDER_NOT_RUN / OWNER_REVIEW_PENDING`

## Purpose

Walk from inputs to a result This is a 6-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-045`
- Sources: `SRC-WEB-003`
...
|---|---|---|---|---|---|
| 0–1s | claim | absent | contrast | THE CLAIM | `frames/SF-045-F01.svg` |
| 1–2s | hide/reveal | absent | contrast | REVEAL | `frames/SF-045-F02.svg` |
| 2–3s | proof close-up | absent | contrast | PROOF | `frames/SF-045-F03.svg` |
| 3–4s | trace | absent | contrast | TRACE | `frames/SF-045-F04.svg` |
| 4–5s | limitation | absent | contrast | LIMIT | `frames/SF-045-F05.svg` |
| 5–6s | verdict | absent | contrast | VERDICT | `frames/SF-045-F06.svg` |

## Production prompt

Create a vertical 9:16 6-second clean-room insert titled “Calculation Reveal.” Use one meaningful visual state per second. Hook mechanism: `reveal`. Viewer job: `compare_options`. Visual grammar: `kinetic_type`. B-roll function: `contrast`. Proof form: `worked_example`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
