...
# Собственный срок шага (14 сентября 2026): истёкший deadline останавливает разбор до
# первого скачивания, поэтому тест не ходит в сеть. Прежде срок был только один — общий
# `timeout` в cron.sh, и он убивал весь прогон вместе с десятью невыполненными шагами.
import time as _time
expired = deep.run(con, rows, deadline=_time.time() - 1)
eq('истёкший срок: ничего не разобрано', expired[0], 0)
eq('истёкший срок: шаг сообщает, что остановлен', expired[3], True)
eq('без срока разбор не останавливается сам', deep.run(con, [], deadline=None)[3], False)
