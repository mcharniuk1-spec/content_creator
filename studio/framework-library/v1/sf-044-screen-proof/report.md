...
Show a deterministic screen state This is a 5-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-044`
- Sources: `SRC-WEB-003`
...
|---|---|---|---|---|---|
| 0–1s | claim | voice_over | prove_state | THE CLAIM | `frames/SF-044-F01.svg` |
| 1–2s | hide/reveal | voice_over | prove_state | REVEAL | `frames/SF-044-F02.svg` |
| 2–3s | proof close-up | voice_over | prove_state | PROOF | `frames/SF-044-F03.svg` |
| 3–4s | limitation | voice_over | prove_state | LIMIT | `frames/SF-044-F04.svg` |
| 4–5s | verdict | return_cut | prove_state | VERDICT | `frames/SF-044-F05.svg` |

## Production prompt

Create a vertical 9:16 5-second clean-room insert titled “Screen Proof.” Use one meaningful visual state per second. Hook mechanism: `reveal`. Viewer job: `learn_mechanism`. Visual grammar: `layered_pip`. B-roll function: `prove_state`. Proof form: `constraint`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
