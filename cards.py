...
# Вторая: тема для разработчика. Наша аудитория — уровень 0–6 из десяти, и плейбук
# прямо говорит, что мы не обслуживаем тех, кому нужны локальные модели, оркестрация
# и мультиагентные архитектуры. Такой ролик может быть отличным по метрикам и всё
# равно не нашим.
DEV_TOPICS = {
...
def _pool(con, today):
    ...
    for r in rows:
        ...
        if set(topics) & OFF_TOPICS:              # не наш жанр
            continue
        # ролик целиком про инструментарий разработчика: для нашего зрителя это не тема.
        # Если у ролика есть и «наша» тема помимо технических, он остаётся в пуле.
        if topics and set(topics) <= (DEV_TOPICS | NOT_TOPICS):
            continue
