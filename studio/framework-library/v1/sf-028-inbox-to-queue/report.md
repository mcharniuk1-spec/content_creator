# SF-028 — Inbox to Queue

Truth state: `ORIGINAL_LOCAL_PREVISUALIZATION / EXTERNAL_PROVIDER_NOT_RUN / OWNER_REVIEW_PENDING`

## Purpose

Convert reactive intake into controlled flow This is a 7-second reusable hook/intro/B-roll insert, not a final video and not a performance claim.

## Signal provenance

- Export: `SIGEXP-SF-028`
- Sources: `SRC-WEB-004`
...
|---|---|---|---|---|---|
| 0–1s | before | picture_in_picture | attention_reset | BEFORE | `frames/SF-028-F01.svg` |
| 1–2s | friction | picture_in_picture | attention_reset | FRICTION | `frames/SF-028-F02.svg` |
| 2–3s | change action | picture_in_picture | attention_reset | CHANGE | `frames/SF-028-F03.svg` |
| 3–4s | after | picture_in_picture | attention_reset | AFTER | `frames/SF-028-F04.svg` |
| 4–5s | limitation | picture_in_picture | attention_reset | LIMIT | `frames/SF-028-F05.svg` |
| 5–6s | bridge detail | picture_in_picture | attention_reset | BRIDGE | `frames/SF-028-F06.svg` |
| 6–7s | hold comparison | picture_in_picture | attention_reset | COMPARE | `frames/SF-028-F07.svg` |

## Production prompt

Create a vertical 9:16 7-second clean-room insert titled “Inbox to Queue.” Use one meaningful visual state per second. Hook mechanism: `before_after`. Viewer job: `assess_risk`. Visual grammar: `split_screen`. B-roll function: `attention_reset`. Proof form: `process_trace`. Keep all captions, labels, UI-like elements, and proof callouts as editable deterministic overlays. Maintain an 8% horizontal and 10% vertical planning inset, then verify the actual platform UI safe zone at production. Use only original or licensed footage and audio. Do not copy creator wording, source frames, layouts, music, gestures, identity, branded UI, or distinctive sequence. External image/video/audio provider execution remains NOT RUN.
